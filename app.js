...
/* ================================================================
   ...
   ----------------------------------------------------------------
   buildCaseGroup() now returns the full case with all components
   mounted inside, all driven by bapc state.

   Coordinate system:
     +X = right side of case (glass panel)
     -X = left side of case (mobo tray, motherboard mounts here)
     +Y = up
     -Y = down
     +Z = front of case
...
function buildCaseGroup(caseData){
  ...
  addBox(0.02, h - 0.1, d - 0.1,
    new THREE.MeshPhysicalMaterial({
      color: 0x88aaff, metalness: 0, roughness: 0.05,
      transmission: 1.0, transparent: true, opacity: 0.18,
      thickness: 0.02, ior: 1.4
    }),
    halfW - 0.01, 0, 0, 'side-glass');
  const glassPanel = group.children[group.children.length - 1];
  glassPanel.renderOrder = 10;

  /* --- solid opposite side (mobo tray) --- */
  ...
  /* ============================================================
     PART 2 — MOTHERBOARD + CPU + RAM + COOLER + GPU + PSU + STORAGE
     ============================================================ */
  ...
  // Mobo sits on the -X side, standing vertically in the YZ plane.
  // From the +X camera, we see its face.
  ...
  const moboSize = moboSizes[moboForm] || moboSizes.ATX;
  const moboW = 0.04;                       // PCB thickness
  const moboX = -halfW + 0.08;              // just inside the solid panel
  const moboY = shroudTop + moboSize.h/2 + 0.15;
  const moboZ = 0;

  // PCB
  const moboMat = new THREE.MeshStandardMaterial({ color: 0x1c2a24, metalness: 0.3, roughness: 0.75 });
  addBox(moboW, moboSize.h, moboSize.d, moboMat, moboX, moboY, moboZ, 'mobo-pcb');

  // VRM heatsinks (top-left and top-right of mobo, from mobo's own orientation)
  const heatsinkMat = new THREE.MeshStandardMaterial({ color: 0x2a2f38, metalness: 0.85, roughness: 0.3 });
  addBox(0.06, 0.14, 0.5, heatsinkMat, moboX + 0.05, moboY + moboSize.h/2 - 0.12, -0.25);
  addBox(0.06, 0.14, 0.5, heatsinkMat, moboX + 0.05, moboY + moboSize.h/2 - 0.12,  0.25);

  // I/O shroud (top edge)
  addBox(0.06, 0.16, moboSize.d * 0.55, heatsinkMat, moboX + 0.06, moboY + moboSize.h/2 - 0.1, -moboSize.d/4);

  // Chipset heatsink (lower area)
  addBox(0.05, 0.22, 0.22, heatsinkMat, moboX + 0.05, moboY - moboSize.h/2 + 0.35, 0.2);

  /* ---- CPU socket + CPU ---- */
  const socketW = 0.34, socketD = 0.34;
  const socketX = moboX + 0.05;
  const socketY = moboY + moboSize.h/2 - 0.55;
  const socketZ = -0.1;

  const socketMat = new THREE.MeshStandardMaterial({ color: 0x15181e, metalness: 0.5, roughness: 0.6 });
  addBox(0.05, socketW, socketD, socketMat, socketX, socketY, socketZ, 'socket');

  // CPU IHS — the visible shiny chip
  const cpuMat = new THREE.MeshStandardMaterial({ color: 0xd4d8de, metalness: 0.95, roughness: 0.15 });
  addBox(0.06, socketW * 0.75, socketD * 0.75, cpuMat, socketX + 0.03, socketY, socketZ, 'cpu-ihs');

  // CPU triangle marker (little corner dot)
  const markerMat = new THREE.MeshStandardMaterial({ color: 0xfbbf24, emissive: 0xfbbf24, emissiveIntensity: 0.4 });
  addBox(0.062, 0.03, 0.03, markerMat, socketX + 0.031, socketY + socketW*0.3, socketZ - socketD*0.3);

  /* ---- RAM sticks ---- */
  // Mobo has 2 or 4 DIMM slots. Number of sticks depends on capacity chosen.
  ...
  const ramStickH = 0.65;
  const ramStickD = 0.05;
  const ramStickW = 0.05;
  const ramSpacing = 0.08;
  const ramBaseY = moboY + moboSize.h/2 - 0.55;
  const ramBaseZ = socketZ + socketD/2 + 0.15;
  const ramMat = new THREE.MeshStandardMaterial({ color: 0x2b3038, metalness: 0.7, roughness: 0.35 });
  const ramPCBMat = new THREE.MeshStandardMaterial({ color: 0x0d1017, metalness: 0.2, roughness: 0.9 });
  const ramGoldMat = new THREE.MeshStandardMaterial({ color: 0xfbbf24, metalness: 0.95, roughness: 0.25 });

  // Draw the slots (even empty ones) so it looks like a real board
  for(let i = 0; i < ramSlots; i++){
    const z = ramBaseZ + i * ramSpacing;
    // slot
    addBox(0.06, 0.08, 0.04, ramPCBMat, moboX + 0.04, ramBaseY, z, 'dimm-slot-' + i);
    // stick (only if user has this many)
    if(i < ramStickCount){
      // heatspreader
      addBox(ramStickW, ramStickH, ramStickD, ramMat, moboX + 0.06, ramBaseY + ramStickH/2, z, 'ram-' + i);
      // gold contact strip at bottom
      addBox(0.055, 0.06, ramStickD - 0.005, ramGoldMat, moboX + 0.062, ramBaseY + 0.03, z);
    }
  }

  /* ---- PCIe slots + GPU ---- */
  // PCIe x16 slot
  const pcieX = moboX + 0.05;
  const pcieY = moboY - 0.15;
  const pcieZ1 = 0.05;
  const pcieZ2 = -0.35;
  const pcieMat = new THREE.MeshStandardMaterial({ color: 0x1e2836, metalness: 0.7, roughness: 0.4 });
  addBox(0.05, 0.05, 0.7, pcieMat, pcieX, pcieY, pcieZ1, 'pcie-top');
  addBox(0.05, 0.05, 0.5, pcieMat, pcieX, pcieY - 0.15, pcieZ2, 'pcie-mid');

  // GPU — mounted in the top PCIe slot, extends toward +X (visible through glass)
  if(gpu){
    // size by tier
    const gpuTier = gpu.tier;
    const gpuLen  = gpuTier === 'flagship' ? 1.6 : gpuTier === 'enthusiast' ? 1.4 : gpuTier === 'performance' ? 1.2 : 1.0;
    const gpuThick = gpuTier === 'flagship' ? 0.25 : gpuTier === 'enthusiast' ? 0.2 : 0.16;
    const gpuHeight = gpu.vram >= 16 ? 0.28 : 0.22;
    const fanCount = gpu.tdp > 250 ? 3 : 2;

    const gpuX = moboX + 0.15 + gpuThick/2;
    const gpuY = pcieY + gpuHeight/2 - 0.02;
    const gpuZ = pcieZ1;

    // shroud
    const gpuShroudMat = new THREE.MeshStandardMaterial({ color: 0x2a2f38, metalness: 0.7, roughness: 0.4 });
    addBox(gpuLen, gpuHeight, gpuThick, gpuShroudMat, gpuX + gpuLen/2, gpuY, gpuZ, 'gpu-shroud');

    // fans on the +X face (visible through glass)
    const fanMat  = new THREE.MeshStandardMaterial({ color: 0x0a0d14, metalness: 0.4, roughness: 0.8 });
    const fanBladeMat = new THREE.MeshStandardMaterial({ color: 0x3a4255, metalness: 0.6, roughness: 0.4 });
    const fanRadius = gpuHeight * 0.4;
    const fanSpacing = gpuLen / (fanCount + 1);
    for(let f = 0; f < fanCount; f++){
      const fx = gpuX + fanSpacing * (f + 1);
      // fan ring
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(fanRadius, 0.015, 8, 24),
        fanBladeMat
      );
      ring.position.set(fx, gpuY, gpuZ + gpuThick/2 + 0.005);
      ring.rotation.y = Math.PI/2;
      ring.castShadow = true;
      group.add(ring);
      // fan hub
      addCylinder(fanRadius * 0.15, 0.03, fanMat, fx, gpuY, gpuZ + gpuThick/2 + 0.01, Math.PI/2);
    }

    // GPU bracket at rear (-Z)
    const bracketMat = new THREE.MeshStandardMaterial({ color: 0x8a94a8, metalness: 0.9, roughness: 0.25 });
    addBox(0.12, gpuHeight + 0.05, 0.04, bracketMat, gpuX + 0.05, gpuY, gpuZ - 0.4);

    // power connector (top)
    const powerMat = new THREE.MeshStandardMaterial({ color: 0x0d1017, metalness: 0.3, roughness: 0.9 });
    addBox(0.12, 0.06, 0.14, powerMat, gpuX + gpuLen*0.6, gpuY + gpuHeight/2 + 0.01, gpuZ);
  }

  /* ---- CPU Cooler ---- */
  if(cooler && cpu){
    const coolerStyle = cooler.style || 'air-single';
    const coolerMat = new THREE.MeshStandardMaterial({ color: 0x8a94a8, metalness: 0.85, roughness: 0.3 });
    const fanCoolerMat = new THREE.MeshStandardMaterial({ color: 0x0d1017, metalness: 0.3, roughness: 0.85 });

    const cX = socketX + 0.05;
    const cY = socketY;
    const cZ = socketZ;

    if(coolerStyle === 'stock'){
      // small cube on top of CPU
      addBox(0.1, 0.28, 0.28, coolerMat, cX, cY, cZ, 'cooler-stock');
    } else if(coolerStyle === 'air-single' || coolerStyle === 'air-low'){
      // fin stack
      const stackH = coolerStyle === 'air-low' ? 0.25 : 0.6;
      addBox(0.12, stackH, 0.34, coolerMat, cX, cY, cZ, 'cooler-single');
      // fan
      addCylinder(0.16, 0.06, fanCoolerMat, cX + 0.1, cY, cZ, Math.PI/2);
    } else if(coolerStyle === 'air-dual'){
      const stackH = 0.65;
      addBox(0.12, stackH, 0.34, coolerMat, cX, cY, cZ - 0.2, 'cooler-dual-1');
      addBox(0.12, stackH, 0.34, coolerMat, cX, cY, cZ + 0.2, 'cooler-dual-2');
      addCylinder(0.16, 0.06, fanCoolerMat, cX + 0.1, cY, cZ - 0.05, Math.PI/2);
    } else if(coolerStyle === 'aio'){
      // pump block on CPU
      addBox(0.12, 0.22, 0.22, fanCoolerMat, cX, cY, cZ, 'aio-pump');
      // radiator at top
      const radMat = new THREE.MeshStandardMaterial({ color: 0x2a2f38, metalness: 0.8, roughness: 0.35 });
      addBox(0.4, 0.05, moboSize.d * 0.8, radMat, -halfW + 0.5, halfH - 0.12, 0, 'aio-rad');
    } else if(coolerStyle === 'custom'){
      addBox(0.14, 0.24, 0.24, fanCoolerMat, cX, cY, cZ, 'custom-block');
    }
  }

  /* ---- PSU ---- */
  if(psu){
    const psuLen = psu.wattage >= 1000 ? 0.9 : psu.wattage >= 750 ? 0.75 : 0.6;
    const psuWidth = 0.5;
    const psuHeight = 0.4;
    const psuMat = new THREE.MeshStandardMaterial({ color: 0x181d28, metalness: 0.6, roughness: 0.55 });

    // if shroud present, PSU hidden inside it (still draw it, just occluded)
    const psuY = -halfH + psuHeight/2 + 0.1;
    const psuX = 0;
    const psuZ = -halfD + psuLen/2 + 0.2;

    addBox(psuWidth, psuHeight, psuLen, psuMat, psuX, psuY, psuZ, 'psu-body');

    // fan grille on the top face
    const grille = new THREE.Mesh(
      new THREE.TorusGeometry(0.14, 0.012, 6, 20),
      new THREE.MeshStandardMaterial({ color: 0x3a4255, metalness: 0.9, roughness: 0.25 })
    );
    grille.position.set(psuX, psuY + psuHeight/2 + 0.005, psuZ);
  ...
  /* ---- Storage ---- */
  // 2.5" drives: flat slabs on the case floor, front of the case
  // M.2 drives: sit on the mobo, we draw one hidden behind the GPU typically
  // For simplicity: place 2.5" slabs for HDD/SATA, hide M.2 (it's on the board)
  if(storage){
    const isM2 = (storage.family || '').includes('NVMe') || (storage.family || '').includes('SATA-M2');
    if(!isM2){
      const driveMat = new THREE.MeshStandardMaterial({ color: 0x232832, metalness: 0.7, roughness: 0.4 });
      const driveH = 0.07, driveW = 0.35, driveD = 0.5;
      for(let i = 0; i < storageQty; i++){
        const dx = -halfW + 0.5 + (i * 0.55);
        addBox(driveW, driveH, driveD, driveMat, dx, shroudTop + 0.06, halfD - 0.4, 'drive-' + i);
      }
    } else {
      // M.2 stick lying flat on the mobo, below the GPU
      const m2Mat = new THREE.MeshStandardMaterial({ color: 0x1a2030, metalness: 0.5, roughness: 0.5 });
      addBox(0.03, 0.05, 0.5, m2Mat, moboX + 0.045, moboY - 0.55, 0.1, 'm2');
    }
  }
