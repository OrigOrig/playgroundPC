...
/* ================================================================
   UPGRADE SIMULATOR — sandboxed
   Runs analyze() against a copy of state.build so nothing leaks.
   Only "Save as My Build" commits.
   ================================================================ */
const sim = {
  build: null,        // deep copy of state.build when opened
  analysis: null,     // result of analyze() against sim.build
  lastRun: null       // snapshot of state.build at the time sim.build was seeded
};
...
/* ---------- Seed the sandbox from the real build ---------- */
function simSeed(){
  // If there's no real build, treat it as empty
  ...
  sim.build = deepClone(state.build);
  sim.analysis = null;
  sim.lastRun = deepClone(state.build);
}
...
function simPopulateSelects(){
  ...
  // CPU
  const allCpuList = sortCpus(allCpus());
  ...
  $gpu.innerHTML = `<option value="" disabled>—</option>` +
    allGpuList.map(g => `<option value="${g.name}">${g.name} · ${g.vram}GB</option>`).join('');
...
/* ---------- Read the four comboboxes into sim.build ---------- */
function simReadOverrides(){
  ...
  if(stgVal){
    sim.build.storages = sim.build.storages || [];
    if(sim.build.storages.length === 0){
      sim.build.storages.push({ type: stgVal, capacity: '1TB' });
    } else {
...
/* ---------- Run analyze() against the sandbox ---------- */
...
function simRenderResult(before, after){
  ...
  if(!before || !after){
    wrap.innerHTML = `
      <div class="empty">
        ...
        <p>Pick a part above and hit Simulate to see the impact.</p>
      </div>`;
    return;
  }
  ...
  // --- Per-game FPS delta (chart A) ---
  const gameRows = after.gameResults.map(g => {
    const oldG = before.gameResults.find(x => x.name === g.name);
    const oldFps = oldG ? oldG.fps : g.fps;
    const delta = g.fps - oldFps;
    const pct = oldFps > 0 ? (delta / oldFps * 100) : 0;
    return { name: g.name, oldFps, newFps: g.fps, delta, pct };
  });
  // Sort by absolute delta descending, take top 15 for the table
  const topChanged = [...gameRows]
    .filter(g => Math.abs(g.delta) > 0)
  ...
  const avgPct = avgOld > 0 ? (avgDelta / avgOld * 100) : 0;

  // Cost delta
  let costDelta = 0;
  const seed = sim.lastRun || {};
  const oldCpu = getCpuData(seed.cpu);
  const newCpu = getCpuData(sim.build.cpu);
  const oldGpu = getGpuData(seed.gpu);
  ...
  // --- Score ring with ghost (chart D) ---
  const circ = 2 * Math.PI * 54;
  const beforePct = before.totalScore / 100;
  const afterPct = after.totalScore / 100;
  const beforeOffset = circ * (1 - beforePct);
  const afterOffset = circ * (1 - afterPct);

  // --- Before/after component bars (chart B) ---
  const meters = [
    { label:'CPU',     old:before.cpuScore,     neu:after.cpuScore },
    { label:'GPU',     old:before.gpuScore,     neu:after.gpuScore },
    { label:'RAM',     old:before.ramScore,     neu:after.ramScore },
    { label:'Storage', old:before.storageScore, neu:after.storageScore }
  ];
  ...
  wrap.innerHTML = `
    ...
    <div class="grid grid-2 mb-3" style="gap:1.5rem;align-items:center;">
      ...
      <!-- Score ring with old-score ghost -->
      <div style="display:flex;align-items:center;justify-content:center;gap:1.5rem;">
        <div class="score-ring" style="position:relative;width:150px;height:150px;">
          <svg width="150" height="150" viewBox="0 0 150 150" style="transform:rotate(-90deg);">
            ...
            <circle cx="75" cy="75" r="62" fill="none" stroke="var(--surface-3)" stroke-width="12"/>
            <circle cx="75" cy="75" r="62" fill="none" stroke="var(--text-3)" stroke-width="12"
                    stroke-dasharray="${circ.toFixed(1)}"
                    stroke-dashoffset="${(circ * (1 - beforePct)).toFixed(1)}"
                    stroke-linecap="round" opacity="0.35"
                    style="transition:stroke-dashoffset 800ms cubic-bezier(.22,1,.36,1);"/>
            <circle cx="75" cy="75" r="62" fill="none" stroke="url(#simScoreGrad)" stroke-width="12"
                    stroke-dasharray="${(2 * Math.PI * 62).toFixed(1)}"
                    stroke-dashoffset="${((2 * Math.PI * 62) * (1 - afterPct)).toFixed(1)}"
          ...
          <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
            ...
            <div style="font-size:.75rem;color:var(--text-3);font-weight:600;">/ 100</div>
            <div style="font-size:.75rem;font-weight:700;color:${deltaClass};margin-top:.35rem;">
              ${scoreDelta === 0 ? '—' : deltaSign + scoreDelta}
            </div>
          </div>
        </div>
        <div style="display:flex;flex-direction:column;gap:.5rem;">
          <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text-3);">Before</div>
          <div style="font-size:1.4rem;font-weight:800;letter-spacing:-.02em;color:var(--text-2);">${before.totalScore}/100</div>
          <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text-3);margin-top:.5rem;">After</div>
          <div style="font-size:1.4rem;font-weight:800;letter-spacing:-.02em;">${after.totalScore}/100</div>
        </div>
      </div>
      ...
      <div class="grid grid-2" style="gap:1rem;">
        <div class="card-soft" style="background:var(--surface-2);padding:1rem;border-radius:var(--radius-sm);">
          <div class="text-muted" style="font-size:.7rem;text-transform:uppercase;letter-spacing:.06em;font-weight:700;">Avg FPS change</div>
          <div style="font-size:1.6rem;font-weight:800;letter-spacing:-.03em;line-height:1;color:${avgDelta>=0?'var(--success)':'var(--danger)'};">
            ${avgDelta >= 0 ? '+' : ''}${avgDelta.toFixed(1)}
          </div>
          <div style="font-size:.78rem;font-weight:600;color:var(--text-3);margin-top:.3rem;">
            ${avgPct >= 0 ? '+' : ''}${avgPct.toFixed(1)}% across ${after.gameResults.length} games
          </div>
        </div>
        <div class="card-soft" style="background:var(--surface-2);padding:1rem;border-radius:var(--radius-sm);">
          ...
          <div style="font-size:1.6rem;font-weight:800;letter-spacing:-.03em;line-height:1;">
            ${costDelta === 0 ? '—' : (costDelta > 0 ? '+$' : '-$') + Math.abs(costDelta)}
          </div>
          <div style="font-size:.78rem;font-weight:600;color:var(--text-3);margin-top:.3rem;">
            ${costDelta === 0 ? 'no new cost' : 'parts only'}
          </div>
        </div>
      </div>
        ...
        return `
          <div style="margin-bottom:1rem;">
            <div class="flex-between mb-1" style="font-size:.82rem;">
              ...
              <span class="text-muted" style="display:inline-flex;align-items:baseline;gap:.65rem;">
                <span><strong style="color:var(--text-2);">${m.old}</strong> → <strong style="color:var(--text);">${m.neu}</strong></span>
                <span style="color:${deltaColor};font-weight:700;min-width:44px;text-align:right;">
                  ${delta === 0 ? '—' : (delta > 0 ? '+' : '') + delta}
                </span>
              </span>
            </div>
            <div style="position:relative;height:8px;background:var(--surface-3);border-radius:4px;overflow:hidden;">
              <div style="position:absolute;top:0;left:0;height:100%;width:${m.old}%;background:var(--text-3);opacity:.35;border-radius:4px;transition:width 700ms cubic-bezier(.22,1,.36,1);"></div>
              <div style="position:absolute;top:0;left:0;height:100%;width:${m.neu}%;background:linear-gradient(90deg,var(--primary),var(--accent));border-radius:4px;transition:width 700ms cubic-bezier(.22,1,.36,1);"></div>
            </div>
          </div>`;
      }).join('')}
      <div style="display:flex;gap:1.25rem;justify-content:center;margin-top:1rem;font-size:.72rem;color:var(--text-3);">
        <span><span style="display:inline-block;width:10px;height:10px;background:var(--text-3);opacity:.4;border-radius:2px;vertical-align:middle;margin-right:.35rem;"></span>Before</span>
        <span><span style="display:inline-block;width:10px;height:10px;background:linear-gradient(90deg,var(--primary),var(--accent));border-radius:2px;vertical-align:middle;margin-right:.35rem;"></span>After</span>
      </div>
    </div>
          ...
          <tbody>
            ${topChanged.map(g => `
              <tr>
                ...
                <td style="text-align:right;font-weight:700;color:${g.delta>0?'var(--success)':g.delta<0?'var(--danger)':'var(--text-3)'};">
                  ${g.delta > 0 ? '+' : ''}${g.delta}
                  <span style="font-size:.72rem;opacity:.7;margin-left:.35rem;">(${g.pct>0?'+':''}${g.pct.toFixed(0)}%)</span>
                </td>
              </tr>`).join('')}
          </tbody>
        </table>
    ...
    ` : `
      <div class="card-soft" style="background:var(--surface-2);padding:1.25rem;border-radius:var(--radius-sm);">
        <div class="empty" style="padding:1rem;">
          ...
          <p>No FPS changes from this swap — try a bigger upgrade.</p>
        </div>
      </div>
    `}
  `;
...
/* ---------- Full simulate action ---------- */
function simSimulate(){
  if(!sim.build || !sim.build.cpu || !sim.build.gpu){
    toast('Configure your PC first', 'fa-triangle-exclamation');
    return;
  }

  // Make sure we have a "before" snapshot
  const before = sim.lastRun ? simRunAnalysisForBuild(sim.lastRun) : null;
  // Actually just cache the seed analysis. Simpler: always re-analyze the seed.
  const beforeAnalysis = before || simBaselineAnalysis();
  if(!beforeAnalysis){
    toast('Could not compute baseline', 'fa-triangle-exclamation');
    return;
  }

  // Read the current dropdown values into sim.build
  simReadOverrides();

  // Run analyze() against sim.build
  const afterAnalysis = simRunAnalysis();
  if(!afterAnalysis){
    toast('Simulation failed', 'fa-triangle-exclamation');
    return;
  }

  sim.analysis = afterAnalysis;
  simRenderResult(beforeAnalysis, afterAnalysis);
  simRenderCurrentList();
}

/* ---------- Analyze an arbitrary build without touching state ---------- */
...
function simBaselineAnalysis(){
  if(!sim.lastRun) return null;
  return simRunAnalysisForBuild(sim.lastRun);
}

/* ---------- Revert — discard sandbox ---------- */
function simRevert(){
  simSeed();
  simPopulateSelects();
  simRenderCurrentList();
  $('#simResult').innerHTML = `
    <div class="empty">
      <i class="fas fa-flask"></i>
      <p>Reverted. Pick a part above and hit Simulate to try again.</p>
    </div>`;
  toast('Reverted to your real build', 'fa-arrow-uturn-left');
}

/* ---------- Save as My Build — commit sandbox ---------- */
function simSave(){
  if(!sim.build){
    toast('Nothing to save', 'fa-triangle-exclamation');
    return;
  }
  if(!sim.analysis){
  ...
  const name = prompt('Name your simulated build:',
    (sim.build.cpu || 'My').split(' ')[0] + ' + ' + (sim.build.gpu || 'Build').split(' ').slice(0,2).join(' '));
  if(!name) return;

  // Save the build entry
  const entry = {
    id: Date.now(),
    name,
    build: deepClone(sim.build),
  ...
  // Commit to the real build
  state.build = deepClone(sim.build);
  CK.set('pcp_build', state.build);

  // Now run a real analyze so everything updates
  analyze();

  // Refresh simulator seed
  simSeed();
  simPopulateSelects();
  simRenderCurrentList();

  toast('Saved "' + name + '" and updated My PC', 'fa-save');
}
...
function renderSimulatorPage(){
  ...
  // Seed the sandbox only when the page is opened fresh or after a commit
  if(!sim.build || !sim.lastRun || sim.lastRun.cpu !== state.build.cpu || sim.lastRun.gpu !== state.build.gpu){
    simSeed();
  }
  simPopulateSelects();
  simRenderCurrentList();

  // Restore last result if there is one
  if(sim.analysis && sim.lastRun){
    const beforeAnalysis = simBaselineAnalysis();
    if(beforeAnalysis) simRenderResult(beforeAnalysis, sim.analysis);
  }
}
